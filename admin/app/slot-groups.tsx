...
type Props={slots:any[],events:any[],experiences:any[],modes:any[],eventId:string,onEventChange:(id:string)=>void,query:string,onEdit:(s:any)=>void,onAdd:(s:any)=>void,onDelete:(s:any)=>void,busy:boolean};
export default function SlotGroups({slots,events,experiences:catalog,modes,eventId,onEventChange,query,onEdit,onAdd,onDelete,busy}:Props){
 ...
 const options=modes.filter(m=>m.event_id===activeEvent?.id).map(m=>({value:m.id,label:m.name}));
 const experiences=[...new Set([...options.map(x=>x.value),...activitySlots.map(s=>s.mode_id)])];
 const selectedExperience=experience!==null&&experiences.includes(experience)?experience:experiences[0]||'';
 const items=catalog.filter(x=>x.mode_id===selectedExperience);
 const direct=modes.find(m=>m.id===selectedExperience)?.kind==='direct';
 const selectedItem=direct?'':items.some(x=>x.id===itemId)?itemId:items[0]?.id||'';
 const experienceSlots=activitySlots.filter(s=>s.mode_id===selectedExperience&&(s.experience_id||'')===selectedItem);
 const selectors=<div className="calendar-context-selectors"><label>活动<select aria-label="选择活动" value={activeEvent?.id||''} disabled={!events.length} onChange={e=>{onEventChange(e.target.value);setExperience(null)}}>{!events.length&&<option value="">暂无活动</option>}{events.map(e=><option key={e.id} value={e.id}>{e.title}</option>)}</select></label><label>参与方式<select aria-label="选择体验" value={selectedExperience} disabled={!experiences.length} onChange={e=>setExperience(e.target.value)}>{!experiences.length&&<option value="">仅登岛参观</option>}{experiences.map(name=><option key={name} value={name}>{options.find(x=>x.value===name)?.label||name||'未配置参与方式'}</option>)}</select></label>{!direct&&<label>体验<select aria-label="选择场次体验" value={selectedItem} onChange={e=>setItemId(e.target.value)}>{!items.length&&<option value="">请先新增体验</option>}{items.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select></label>}</div>;
 return <div className="slot-groups">
  {activeEvent?<SlotCalendar selectors={selectors} key={JSON.stringify([activeEvent.id,selectedExperience])} event={activeEvent} experience={selectedExperience} slots={experienceSlots} query={query} onAdd={s=>onAdd({...s,experienceId:selectedItem})} onEdit={onEdit} onDelete={onDelete} busy={busy||(!direct&&!selectedItem)}/>:<section className="panel slot-activity"><header className="slot-activity-heading">{selectors}</header><div className="empty">{activeEvent?'请在活动管理中新增参与方式和体验，再添加场次。':'请先创建活动。'}</div></section>}
 </div>;
}
function SlotCalendar({selectors,event,experience,slots,query,onAdd,onEdit,onDelete,busy}:{selectors:ReactNode,event:any,experience:string,slots:any[],query:string,onAdd:Props['onAdd'],onEdit:Props['onEdit'],onDelete:Props['onDelete'],busy:boolean}){
 ...
 const add=(date:string)=>onAdd({eventId:event.id,modeId:experience,date,time:'13:00-13:40',capacity:2});
 ...
 return <section className="panel slot-activity">
  <header className="slot-activity-heading"><div>{selectors}<p>{event.start_date} 至 {event.end_date} · 当前月 {rows.length} 场 · 已预约人次 {rows.reduce((n,s)=>n+Number(s.booked),0)} / 总名额 {rows.reduce((n,s)=>n+Number(s.capacity),0)}</p></div></header>
